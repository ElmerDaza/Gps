...
import os
import BDsql as bd
from sourse import excel_bot as eb,funciones_externas as fe
#coding: utf-8
...
def carga_masivo(tabla:str,contenido:list):
    '''
    la caraga masiva recibe dos parametros
    tabla: nombre de la tabla
    contenido: una lista que contiene los datos a registrar en tabla
    '''
    for i in contenido:
        bd.Registrar(i,tabla)
        print('registrado*-*-* ',i)

def registro_masivo_BD(tabla='usuarios'):
    '''
    registro masivos de datos almacenados en la 
    hoja de calculo donde estan los datos de usuarios
    recibe un paramero que es el nombre de la taba
        tabla: nombre de la tabla a consultar que por defecto es usuarios
    '''
    direccion= os.getcwd()
    direccion+="/Archivos/"
    ID_FILE=fe.m_.BW.ID_FILE
    eb.file_name=direccion+fe.m_.BW.do.bajar_archivo_por_id(ID_FILE,direccion)
    eb.Nombre_Archivo=fe.m_.BW.Nombre_Archivo
    eb.ID_file=ID_FILE
    carga_masivo(tabla,eb.function_register())
...
def cantidad(A):
    ...
    cantidades = []
    for element in A:
        if(element == '.' or element == '0' or element == '1' or element == '2'
                or element == '3' or element == '4' or element == '5'
                or element == '6' or element == '7' or element == '8' or element == '9' or element == '-'):
            cadena += element
